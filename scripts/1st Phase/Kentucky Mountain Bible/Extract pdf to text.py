...
with open('kentucky-mountain-bible.txt', 'r', encoding='utf-8') as txt_file:
    ...
    for match in matches2:
        ...
        if code not in course_codes:
            ...
            titles = [t.strip() for t in title_parts]
            title = ', '.join(titles[:-1])
            additional_info = titles[-1]
            credits = match[2] if match[2] else '0'
            description = match[3].strip()
            course_data.append({'Code': code, 'Title': title, 'Additional Info': additional_info, 'Credits': credits, 'Description': description})
            course_codes.add(code)
...
with open('kentucky-mountain-bible.csv', 'w', newline='', encoding='utf-8') as csv_file:
    csv_writer = csv.DictWriter(csv_file, fieldnames=['Code', 'Title', 'Additional Info', 'Credits', 'Description'])
