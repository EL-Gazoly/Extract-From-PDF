...
# Open the text file
with open('./taylor-university.txt', 'r', encoding='utf-8') as txt_file:
    txt_content = txt_file.read()

    # Create a regex pattern to match course data
    pattern = re.compile(r'([A-Z]+\s(?:\d+|xxx))\s+(\d+-\d+|\d+)\s+hour[s]?\s+(.*?)(?=\n[A-Z]+\s(?:\d+|xxx)|$)', re.DOTALL)
...
with open('taylor-university.csv', 'w', newline='', encoding='utf-8') as csv_file:
    ...
    for course in course_data:
        ...
        course_row = [course['name'], f"{course['credit hours']} - {course['code']}", course['description']]
