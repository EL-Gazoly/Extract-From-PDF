...
# Open the text file
with open('./houghton-university.txt', 'r', encoding='utf-8') as txt_file:
    txt_content = txt_file.read()

    # Create a regex pattern to match each course individually
    pattern = re.compile(r'(\w+(?: \w+)? (?:\d+(?:, ?\d+)*(?:- \w+(?: \w+)?)?)+) ([^\n]+)\n((?:\d+(?:, ?\d+)*(?:- \w+(?: \w+)?)?)+-[^\n]+)\n((?:(?!\w+(?: \w+)? (?:\d+(?:, ?\d+)*(?:- \w+(?: \w+)?)?)+).+?(?=\w+(?: \w+)? (?:\d+(?:, ?\d+)*(?:- \w+(?: \w+)?)?)+)|\Z))', re.DOTALL)
...
for course in course_data:
    course['credit hours'] = course['credit hours'].split('-')[0].strip()
