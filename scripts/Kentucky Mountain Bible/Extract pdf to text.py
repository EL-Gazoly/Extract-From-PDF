...
with open('kentucky-mountain-bible.csv', 'w', newline='', encoding='utf-8') as csv_file:
    ...
    for course in course_data:
        # Split the title by "(" to separate title and additional information
        title_parts = course['Title'].split('(')
        title = title_parts[0].strip()
        
        # Extract credits from the additional information
        credits_info = title_parts[-1].strip(') ').split()
        credits = credits_info[0]
